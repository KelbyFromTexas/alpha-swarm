...
def score_one(p, largest, age_min, init_buy):
    ...
    if surv is not None and not tape_backed:
        if surv >= 9.5:
            surv = 6.0
        notes.append("surv_capped_not_fully_tape_backed")
    rug_flag = bool(hard_fails)
    rec = "kill"
    if not rug_flag and surv is not None and surv >= 6.5 and age_min is not None and 2 <= age_min <= 45 and tape_backed:
        rec = "advance"
    elif not rug_flag and surv is not None and surv >= 6.5:
        rec = "watch"
        if not tape_backed:
            notes.append("tape_missing_prefer_watch")
        elif age_min is None or not (2 <= age_min <= 45):
